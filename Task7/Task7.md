...
Онлайн-магазин «Мобильный мир» пережил значительный рост: ассортимент расширился с аксессуаров до широкого спектра электроники, аудио- и бытовой техники. Бэкенд, построенный на микросервисной архитектуре с MongoDB в качестве основного хранилища, столкнулся с проблемами производительности в периоды пиковых нагрузок (например, «чёрная пятница»). Анализ показал, что:

- **Высокая нагрузка на запись:** до 5000 заказов/мин и 15000 обновлений корзин/мин.
- **Неравномерное распределение данных:** категория «Электроника» составляет 70% всех запросов к товарам.
- **Операционные риски:** единая база данных (даже с репликами) не справляется с масштабированием записи.

Для решения проблем масштабируемости и производительности принято решение внедрить горизонтальное шардирование MongoDB. Данные будут распределены по нескольким независимым серверам (шардам). Каждый шард будет представлять собой набор реплик (replica set) для обеспечения высокой доступности и отказоустойчивости.

### **Решение**

#### [Диаграмма компонентов C4](https://github.com/AlexanderMATI/architecture-pro-black_friday/blob/main/Task7/Task7.puml)

Данное решение позволяет распределить не только данные, но и нагрузку на запись и чтение, что напрямую решает проблему, возникшую в период пиковых нагрузок.
